...
The function below creates a data frame containing three variables: `expected`, `lower_pi` and `upper_pi` and plots these results to see if there are any weeks in 2009 (the last 52 weeks in the data set) when the number of deaths appears high.[^predintplot]

[^predintplot]: A commented version of this function is given in an appendix to explain what this function does in more detail. 

```{r predintplot}
predintplot <- function(tsdata, tsvar, num_timepts, width,
                        downweight=FALSE, 
                        add=FALSE, ...) { 
  if (width %% 2 == 0) stop('width should be an odd number.')
  results <- data.frame(expected = rep(NA, num_timepts),  
                        lower_pi = rep(NA, num_timepts),  
                        upper_pi = rep(NA, num_timepts)) 
  tsvar <- tsdata[, tsvar]
  startpt <- length(tsvar) - num_timepts + 1
  endpt <- length(tsvar)
...
Note: using `paste0` rather than `paste` ensures no spaces between the concatenated YYYY-mm-dd format (if there are spaces between the date elements the conversion will not work).  This is equivalent to `paste(..., sep = "")`, but slightly more efficient.
...
The date has failed to convert because it does not exist (likely a typographic error); 2010 was not a leap year, so there were only 28 days in February. To correct the date to **28** February 2010, proceed as below, repeating the date conversion and use `summary` to confirm that the problem has been resolved:
...
If desired, a 2 x 2 table can be created using `dcast` from the reshape2 or data.table packages.  The example below illustrates how to construct a table using the data.table package, as the syntax is more efficient (no need to type variable names).
...
Import threesources.dta into R with the `haven` package and browse it to be familiar with the information you need for this type of analysis. You only need to know which individuals selected according to the case definition are included in each source.
...
In R, several packages have been developed to handle capture-recapture data.  These are traditionally used for ecological data, but the principal is the same.  The R package with the closest functionality to the recap STATA ado package is called `Rcapture`.  
...
In the first model, all three data sources are assumed to be independent of each other.  Construct this model with standard formula notation using the `closedPCI` function (for closed populations, with confidence intervals):
...
The Rcapture package alternately facilitates the use of summary tables to construct the models (equivalent to the `collapse` step in STATA).  To test this, proceed as below:
...
## The `predintplot` function

For those interested, more details are given below on what the `predintplot` function does. This is quite conceptually dense in coding terms, well beyond the scope of this training and probably only of interest to advanced R coders. Detailed understanding of how this function works is not required for the purposes of this course; in fact, this code was presented as a function to avoid course participants having to delve into complex code, and also to allow them to easily re-run the function to see the effect of changing the arguments. 

We created a function called `predintplot` which has four required and two optional arguments. The arguments are:

- `tsdata`: A `zoo` time series
- `tsvar`: the name of the variable of interest in the time series (in quotation marks)
- `num_timepts`: the number of time points at the end of the time series for which predictions are required
- `width`: the width of the time window to use for predictions (an odd number)
- `downweight`: whether to downweight outliers in model fitting
- `add`: whether to plot onto a previous plot
- the `...` (ellipsis) allows other arguments to be given, which here will be supplied to the `plot` function

```{r predintplot01, eval=FALSE}
predintplot <- function(tsdata, tsvar, num_timepts, width,
                        downweight=FALSE, 
                        add=FALSE, ...) { 
```

We check if the `width` argument is an odd number using the modulus (`%%`) function, checking whether `width` divided by 2 has a zero remainder; in other words whether `width` is even. If so, the function `stop`s with the given error message. 

We want `width` to be odd so that the window of data we use for prediction contains a time point and an equal number of observations on each side of that time point. 

```{r predintplot02, eval=FALSE}
  if (width %% 2 == 0) stop('num_timepts should be an odd number.')
```

We then set up an empty data frame to hold the results. When a function is creating an object such as a data frame, it is good practice with R (i.e. the code runs quicker) if you create an empty object of the size required to begin with, rather than starting with a small object and adding to it gradually.

```{r predintplot03, eval=FALSE}
  results <- data.frame(expected = rep(NA, num_timepts),  
                        lower_pi = rep(NA, num_timepts),  
                        upper_pi = rep(NA, num_timepts)) 
```

We then extract the variable of interest and call it `tsvar`.

```{r predintplot04, eval=FALSE}
  tsvar <- tsdata[, tsvar]
```

We then find out the index number of the start of the period for which we require predictions. We do this by first finding the index number of the final observation (using `length`), subtracting `num_timepts` from this (which gives us the index number of the final time point in the data to be used for prediction) and then adding 1 to give us finally the index number of the first time point in the time series for which predictions are required. We call this `startpt`.

```{r predintplot05, eval=FALSE}
  startpt <- length(tsvar) - num_timepts + 1
```

We then find the index number of the end of the part of the time series for which predictions are required and call this `endpt`. 

```{r predintplot06, eval=FALSE}
  endpt <- length(tsvar)
```

So by this point we have a single time series variable and have identified the start and end points of the (final) part of it for which we require predictions. In the example we make predictions for the final 52 weeks. 

For each time point in the final period we will fit a model based on the historical data and plot the prediction intervals. Let's take the example of week 10 in the final period. Because there is seasonality in the time series, we would want to fit a model based only on historical data from around previous week 10s. For the purposes of this exercise, we fit the model based on five historical observations centred on week 10, i.e. weeks 8, 9, 10, 11 and 12 (you could use a wider window by altering the `width` argument). This is what the next few lines of code is doing.

The (inbuilt) `cycle` function (see `?cycle`) gives us the week number of each timepoint in `tsvar`. We save this as `cycles` as we will need this in a moment.

```{r predintplot07, eval=FALSE}
  cycles <- as.numeric(cycle(tsvar))
```

We then create a variable called `oneside` which holds the number of observations we want to include in our window on each side of the time point. If we are using windows of width 5, then there are 2 time points on each side. It should hopefully begin to make more sense why we are doing this in a moment.

```{r predintplot08, eval=FALSE}
  oneside <- (width-1)/2
```

We now begin to loop through each of the time points for which we require predictions, with `j` indicating the index of each time point.

```{r predintplot09, eval=FALSE}
  for (j in startpt:endpt){  
```

The next line of code is key. This uses `cycles`, `oneside` and `j` to create an indicator variable, `h`, which is `TRUE` for all time points in the data corresponding to the window of interest, and `FALSE` otherwise.  Let's break it down a bit.

In the example we ran earlier, we had ten years of data and wanted to use the first nine years to make predictions for the final year. Week 1 of the 10th year is observation 469 ($9\times 52 + 1 = 469$). So at the first run through the loop, `j` is 469. 

If `j` is 469, `(j-oneside):(j+oneside)` gives us a vector containing 467, 468, 469, 470 and 471. 

We then use this as an index number to obtain the week numbers we are interested in. `cycles[(j-oneside):(j+oneside)]` gives us a vector containing 51, 52, 1, 2 and 3. 

We can now flag up the windows of weeks around week 1 in all the data. The `%in%` operator (used as `x %in% y`) looks whether each value of `x` matches a value in `y`. So `cycles %in% cycles[(j-oneside):(j+oneside)]` gives us a vector of `TRUE` and `FALSE` of the same length as the data, indicating which values correspond to the window around week 1, which we will use to select data for the regression model.

```{r predintplot10, eval=FALSE}
    h <- cycles %in% cycles[(j-oneside):(j+oneside)]
```

We don't want to use any of the data in the final year in our model, so we make sure the indicator variable is `FALSE` for all of those observations.

```{r predintplot11, eval=FALSE}
    h[startpt:endpt] <- FALSE
```

If you have got this far and still feel like you understand the code you are doing well! We now have an indicator variable, `h`, which we can use to subset the data we will fit the model with. In fact, that is what we do in the next line of code. We fit a null model; i.e. a model with no explanatory variables (only an intercept).

```{r predintplot12, eval=FALSE}
    nullmodel <- lm(tsvar[h] ~ 1)
```

The following `if` clause refits the model as a weighted model (when `downweight` is `TRUE`), using weights derived from the residuals of the previous model. 

```{r predintplot13, eval=FALSE}
    if(downweight){ 
      stdres <- residuals(nullmodel) / sigma(nullmodel)
      wts <- ifelse(stdres > 2, stdres^-2, 1)
      nullmodel <- lm(tsvar[h] ~ 1, weights=wts)
...
The next line of code gets the predicted value and prediction interval and slots them into the appropriate point of the empty data frame we created earlier. The code then loops back to the start of the `for` loop and gets to work on the next value of `j`, continuing until predictions have been obtained for the all the time points required.

```{r predintplot14, eval=FALSE}
    results[j-startpt+1, ] <- predict(nullmodel,
                                      newdata=data.frame(1),
                                      interval='prediction')
  }
...
Once models have been fitted for each of the time points requiring predictions, we plot the data. We skip this block of code when `add` is `TRUE`, which allows us to add the prediction intervals to a previous plot produced by `predintplot` on the same data.

```{r predintplot15, eval=FALSE}
  if(!add){  
    plot(tsvar[startpt:endpt],
         type='l',
         ylim=c(min(c(results$lower_pi, tsvar), na.rm=TRUE),
...
We then plot the prediction intervals.

```{r predintplot16, eval=FALSE}
  lines(as.vector(time(tsvar))[startpt:endpt], results$lower_pi,
        col='green', lty=2)
  lines(as.vector(time(tsvar))[startpt:endpt], results$expected,
        col='red', lwd=2)
...
We join the observed data to the predicted data. 

```{r predintplot17, eval=FALSE}
  results$observed <- as.vector(tsvar)[startpt:endpt]
```

Finally, we return the data frame containing observed values, predicted values and prediction intervals invisibly (i.e. without printing it to the console).

```{r predintplot18, eval=FALSE}
  invisible(results)
}
```

Well done for even reading this far. Functions in R are a very powerful way of automating complex calculations but in practice you would never need to write a function like this - they are usually provided to you by packages. 
